...
class WC_Local_Pickup_Plus_Import extends WC_Local_Pickup_Plus_Import_Export  {
	...
	private function import_pickup_location_data( WC_Local_Pickup_Plus_Pickup_Location $pickup_location, array $import_data, $action ) {
		...
		if ( $local_pickup_plus && $pickup_location instanceof WC_Local_Pickup_Plus_Pickup_Location ) {
			...
			$pickup_location->set_name( is_string( $import_data['name'] ) ? sanitize_text_field( $this->unescape_value( $import_data['name'] ) ) : '' );
			...
			$business_hours = ! empty( $import_data['business_hours'] ) ? json_decode( $import_data['business_hours'], true ) : array();
			if ( ! empty( $business_hours ) && $business_hours !== $local_pickup_plus->get_default_business_hours() ) {
				$string_days = array();
				...
				$pickup_location->set_business_hours( $string_days );
			} else {
			...
			$public_holidays = ! empty( $import_data['public_holidays'] ) && is_string( $import_data['public_holidays'] ) ? array_filter( array_map( 'trim', explode( ',', $this->unescape_value( $import_data['public_holidays'] ) ) ) ) : array();
			if ( ! empty( $public_holidays ) && $public_holidays !== $local_pickup_plus->get_default_public_holidays() ) {
