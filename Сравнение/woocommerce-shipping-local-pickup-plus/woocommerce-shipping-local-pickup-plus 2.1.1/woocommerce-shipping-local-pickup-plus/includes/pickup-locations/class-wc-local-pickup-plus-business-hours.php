...
class WC_Local_Pickup_Plus_Business_Hours {
	 ...
	 * @return array|string
	 */
	public function get_schedule( $day = null, $one_line = false ) {

		$schedule = $this->get_value();

		if ( is_numeric( $day ) && isset( $schedule[ $day ] ) ) {

			$opening_hours = $schedule[ (int) $day ];
			$schedule      = array();
	 ...
	 * @return bool
	 */
	public function has_schedule( $day = null ) {

		if ( is_numeric( $day ) ) {
			$day          = (int) $day;
			$availability = isset( $this->schedule[ $day ] ) ? $this->schedule[ $day ] : null;
		} else {
