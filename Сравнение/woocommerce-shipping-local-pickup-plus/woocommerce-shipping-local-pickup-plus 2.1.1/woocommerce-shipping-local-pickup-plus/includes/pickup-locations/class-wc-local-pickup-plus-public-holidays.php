...
class WC_Local_Pickup_Plus_Public_Holidays {
	...
	public function get_calendar( $start_date = null, $end_date = null ) {
		...
		$start_date = null !== $start_date ? $this->parse_date( $start_date ) : false;
		$end_date   = null !== $end_date   ? $this->parse_date( $end_date )   : false;
	...
	public function get_unavailable_dates( $format = 'mysql', $start_date = null, $end_date = null ) {
		...
		$start_date = $this->parse_date( $start_date );
		$end_date   = $this->parse_date( $end_date );

		if ( ! $start_date ) {
			$start_date = (int) current_time( 'timestamp', true );
		}
		...
		$public_holidays = array_values( $this->get_calendar_dates( $format, $start_date, $end_date ) );
		...
		if ( $this->location_id > 0 && ( $pickup_location = wc_local_pickup_plus_get_pickup_location( $this->location_id ) ) ) {
			...
			do {
				...
				if ( ! $business_hours->has_schedule( $day ) ) {
					$public_holidays[] = 'timestamp' === $format ? $current_date : date( $format, $current_date );
				}
