...
class WC_Local_Pickup_Plus_Ajax {
	...
	public function set_package_handling() {
		...
		if ( isset( $_POST['package_id'], $_POST['pickup_location_id'], $_POST['pickup_date'] ) ) {
	...
	public function get_pickup_location_appointment_data() {
		...
		if (    isset( $_POST['location'] )
			...
			if ( $location && 'publish' === $location->get_post()->post_status ) {
				...
				$start_date = $location->get_pickup_lead_time()->get_relative_time( $today );
				$end_date   = $location->get_pickup_deadline()->get_relative_time( $today );
				...
				wp_send_json_success( array(
					'address'           => $address,
					'calendar_start'    => date( 'Y-m-d', $start_date ),
					'calendar_end'      => date( 'Y-m-d', $end_date ),
