...
class WC_Local_Pickup_Plus_Pickup_Location {
	...
	public function __construct( $id ) {
		...
		// set post meta keys: these fields are saved to post meta and not to dedicated tables
		$this->address_meta            = '_pickup_location_address';
		$this->phone_meta              = '_pickup_location_phone';
		$this->price_adjustment_meta   = '_pickup_location_price_adjustment';
		$this->business_hours_meta     = '_pickup_location_business_hours';
		$this->public_holidays_meta    = '_pickup_location_public_holidays';
		$this->lead_time_meta          = '_pickup_location_pickup_appointment_lead_time';
		$this->deadline_meta           = '_pickup_location_pickup_appointment_deadline';
		$this->email_recipients_meta   = '_pickup_location_email_recipients';
		$this->products_meta           = '_pickup_location_products';
	}
	...
	public function get_description() {
		...
		$description = $this->post instanceof WP_Post ? apply_filters( 'the_content', $this->post->post_content ) : '';
	...
	public function set_pickup_lead_time( $amount, $type ) {
		...
		if ( $this->id > 0 ) {
			...
			$success   = update_post_meta( $this->id, $this->lead_time_meta, $lead_time->get_value() );
		}
	...
	public function has_pickup_lead_time() {
		return ! $this->get_pickup_lead_time()->is_null();
	}
	...
	public function set_pickup_deadline( $amount, $type ) {
		...
		if ( $this->id > 0 ) {
			...
			$success  = update_post_meta( $this->id, $this->deadline_meta,$deadline->get_value() );
		}
	...
	public function has_pickup_deadline() {
		return ! $this->get_pickup_deadline()->is_null();
	}
	...
	public function set_price_adjustment( $adjustment, $amount, $type ) {
		...
		if ( $this->id > 0 ) {
			...
			$success = update_post_meta( $this->id, $this->price_adjustment_meta, $price_adjustment->get_value() );
		}
	...
	public function set_email_recipients( $recipients ) {
		...
		if ( $this->id > 0 ) {
			$success = update_post_meta( $this->id, $this->email_recipients_meta, $recipients );
		}
	...
	public function set_products( array $products ) {
		...
		if ( $this->id > 0 ) {
			...
			$success = update_post_meta( $this->id, $this->products_meta, $set_products );
		}
	...
	public function set_product_categories( array $product_categories ) {
		...
		if ( $this->id > 0 ) {
			...
			$success = update_post_meta( $this->id, $this->products_meta, $set_product_categories );
		}
