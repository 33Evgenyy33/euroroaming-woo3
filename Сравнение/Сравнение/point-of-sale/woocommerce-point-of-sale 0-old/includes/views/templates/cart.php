...
<!--			<a href="#" class="add_custom_meta button tips" data-tip="Edit Product"></a>-->
		...
		<td class="line_cost">
			<div class="view">
				<input type="text" class="product_price" placeholder="{{cart_item_data.price}}" value="{{cart_item_data.price}}" data-discountsymbol="currency_symbol" data-percent="0" readonly disabled >
			</div>
		</td>
...
if ( $user_to_add > 0) {
    ...
    <tr class="item new_row" data-customer_id="0">
        ...
        <td class="name"><?php _e('Гость', 'wc_point_of_sale'); ?></td>
        <?php if( isset( $GLOBALS['wc_points_rewards'] ) ){ ?>
...
<script type="text/template" id="tmpl-cart-coupon-code">
    <tr data-coupon="{{coupon_code}}" class="tr_order_coupon order_coupon_{{coupon_code}}">
        <th class="coupon_label"><span class="span_clear_order_coupon"><?php _e('Remove', 'wc_point_of_sale'); ?></span><?php _e('Скидка', 'wc_point_of_sale'); ?></th>
