...
if ( ! class_exists( 'WPSL_Frontend' ) ) {
	...
	class WPSL_Frontend {
		...
		public function find_nearby_locations() {
			...
			if ( $category_id != '' ) {
				...
				foreach ( $store_data as $store ) {
					...
					//file_put_contents( $_SERVER['DOCUMENT_ROOT'] . "/logs/store1.txt", print_r( $ta_id, true )."\r\n", FILE_APPEND | LOCK_EX );
					//file_put_contents( $_SERVER['DOCUMENT_ROOT'] . "/logs/store1.txt", print_r($sim_cards, true )."\r\n", FILE_APPEND | LOCK_EX );
		...
		public function check_sl_shortcode_atts( $atts ) {
			...
			if ( isset( $atts['category_filter_type'] ) && in_array( $atts['category_filter_type'], array(
					'dropdown',
					'checkboxes'
				) ) ) {
...
//            }
		...
		public function get_infobox_settings( $settings ) {
			...
			$infobox_settings = apply_filters( 'wpsl_infobox_settings', array(
				...
				'infoBoxCloseMargin'            => '2px',
				// The margin can be written in css style, so 2px 2px 4px 2px for top, right, bottom, left
