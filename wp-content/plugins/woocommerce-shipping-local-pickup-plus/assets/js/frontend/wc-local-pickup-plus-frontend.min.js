(function() {  'use strict';
  ...
  jQuery(document).ready(function($) {
    var block, initHandlingToggle, initLookupArea, initPickupLocationsSearch, is_blocked, ref, toggleShippingAddressFields, unblock, wc_local_pickup_plus_frontend;
    ...
    initLookupArea = function() {
      ...
      return $lookupAreaDropdown.on('change', function(e) {
        var $cartForm, $checkoutForm, $enabled, $label, data, fieldId, labelData, locationID, lookupField, objectType, singleLocation;
        ...
        if (true !== $enabled.is(':visible')) {
          ...
          singleLocation = lookupField.attr('data-single-pickup-location');
          locationID = singleLocation && parseInt(singleLocation, 10) > 0 ? singleLocation : $(lookupField).val();
    ...
    initPickupLocationsSearch = function() {
      ...
      enhancedMatcher = function(term, text, option) {
        ...
        if (term) {
          ...
          if (!match && option) {
            if (wc_local_pickup_plus_frontend.select2_version === '3.5.3') {
              postcode = String($(option.context).data('postcode')).toLowerCase();
              city = $(option.context).data('city').toLowerCase();
              address = $(option.context).data('address').toLowerCase();
            } else {
              postcode = String($(option.element).data('postcode')).toLowerCase();
              city = $(option.element).data('city').toLowerCase();
              address = $(option.element).data('address').toLowerCase();
            }
            if (postcode && postcode.indexOf(term) > -1) {
              match = true;
            }
            if (!match && city && city.indexOf(term) > -1) {
              match = true;
            }
            if (!match && address && address.indexOf(term) > -1) {
              match = true;
            }
          }
        }
