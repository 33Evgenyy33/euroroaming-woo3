	...
	private function get_pickup_locations_by_address( WC_Local_Pickup_Plus_Address $address, $args = array() ) {
		...
		if ( '' === $country ) {
			...
			$query = "
				...
				WHERE country = %s
				AND state = %s
				AND ( title LIKE %s OR city LIKE %s OR postcode LIKE %s )
				ORDER BY postcode
			";
		}
		...
		if ( '' === $country ) {
			$results = $wpdb->get_results( $wpdb->prepare( "{$query}", "%{$title}%", "%{$city}%", "%{$postcode}%" ), ARRAY_A );
		} else {
			$results = $wpdb->get_results( $wpdb->prepare( "{$query}", $country, $state," %{$title}%", "%{$city}%", "%{$postcode}%" ), ARRAY_A );
		}
