( function( $ ) {
	...
	$.extend( WC_Usage.prototype, {
		value2input : function( value ) {
			...
			this.$input.find( '[name="usage_limit_products"]' ).val( value.usage_limit_products );
		},
		...
		input2value : function() {
			return {
				...
				usage_limit_products : this.$input.find( '[name="usage_limit_products"]' ).val()
			};
		}
	} );
	...
	template += '<div>';
	template += '<label>' + acp_woocommerce_i18n.woocommerce.usage_limit_products + '</label>';
	template += '<input type="text" class="form-control input-sm small-text" name="usage_limit_products">';
	template += '</div>';
