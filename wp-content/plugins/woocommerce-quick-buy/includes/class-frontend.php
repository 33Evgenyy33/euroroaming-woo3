...
class WooCommerce_Quick_Buy_FrontEnd {
	...
	public function generate_button($args){
		...
		if ($get_virtual = get_post_meta($product->get_id(),'_virtual',true) == "yes") return; //$product->id
		$pid = $product->get_id();
		...
		if($tag == 'button'){
			$return .= '<a type="button" id="'.$defined_id.'" '.$defined_attrs.'  class="w-btn style_raised color_primary wcqb_button '.$defined_class.'">'.$label.'</a>';
			$return .= '<script>

jQuery(document).ready(function($) {
    $(".single_add_to_cart_button").addClass("intro");
